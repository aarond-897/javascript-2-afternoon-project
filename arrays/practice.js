...
function divider(numarr) {
  ...
  for (let i = 0; i < numarr.length; i++) {
    if (numarr[i] % 2 === 0) {
      evensArray.push(numarr[i]);
    } else {
      oddsArray.push(numarr[i]);
    }
  }
...
function addTen(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (typeof arr[i] !== "string") {
      plusTen.push(arr[i] + 10);
    } else {
      plusTen.push(parseInt(arr[i]) + 10);
    }
  }
  return plusTen;
}
